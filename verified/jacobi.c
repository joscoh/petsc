...
int main(int argc, char **args)
{
  ...
  */
  ...
  //JOSH - not going to set from options, instead will set type manually. Options database is very annoying
  ...
  */
  ...
  // //JOSH - also not setting from options - added SetType (not sure we need because AIJ is defaul)
  // //PetscCall(MatSetFromOptions(A));
  // PetscCall(MatSetType(A, MATAIJ));
  // PetscCall(MatSetUp(A));

  //JOSH - use MatCreateSeqAIJ, which preallocates memory appropriately (for now, say 3 elements per column)
  ...
  */
  // i=0;
  // PetscCall(MatSetValues(A, 1, &i, 3, col[0], value[0], INSERT_VALUES));
  // i=1;
  // PetscCall(MatSetValues(A, 1, &i, 3, col[1], value[1], INSERT_VALUES));
  // i=2;
  // PetscCall(MatSetValues(A, 1, &i, 3, col[2], value[2], INSERT_VALUES));
  // i=3;
  // PetscCall(MatSetValues(A, 1, &i, 3, col[3], value[3], INSERT_VALUES));
  // i=4;
  // PetscCall(MatSetValues(A, 1, &i, 3, col[4], value[4], INSERT_VALUES));
  for(i = 0; i < n; i++) {
  ...
  */
  ...
  //scale defaults to 1.0 which is what we want
  ...
  */
  ...
  //JOSH - TODO: replace with VecGetValues
  printf("Solution is: (%f, %f, %f, %f, %f)\n", (*((PetscScalar **)(x->data)))[0], (*((PetscScalar **)(x->data)))[1], (*((PetscScalar **)(x->data)))[2], (*((PetscScalar **)(x->data)))[3], (*((PetscScalar **)(x->data)))[4]);
  ...
  //should keep norm, postcondition norm less than tolerance
